...
if window.Debug == 1
    ...
    % else
    [window.Number , window.Rect] = Screen('OpenWindow', windowin, window.BGColor,[0 0 600 600],[],[],window.StereoMode,window.MultiSample, window.ImagingPipeline,kPsychGUIWindow);
    %end
else
    if window.StereoMode == 4
        PsychImaging('PrepareConfiguration');
...
window.DisplaySize  = Screen('DisplaySize', windowin);
